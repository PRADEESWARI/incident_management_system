    ...
    <div className="flex flex-col min-h-screen">
      ...
      <main className="flex-1 p-4 sm:p-6 space-y-5">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard title="Total Incidents" value={summary?.total_incidents ?? '—'} subtitle="All time" icon={<span className="text-lg">◈</span>} color="default" loading={summaryLoading} />
          <StatCard title="Active" value={summary?.active_incidents ?? '—'} subtitle="Needs attention" icon={<span className="text-lg">◉</span>} color="orange" loading={summaryLoading} pulse={(summary?.active_incidents ?? 0) > 0} />
          <StatCard title="Critical P0" value={summary?.critical_incidents ?? '—'} subtitle="Immediate action" icon={<span className="text-lg">⬡</span>} color="red" loading={summaryLoading} pulse={(summary?.critical_incidents ?? 0) > 0} />
          <StatCard title="Resolved" value={summary?.resolved_incidents ?? '—'} subtitle={`MTTR ${formatDuration(summary?.mttr_today_seconds)}`} icon={<span className="text-lg">○</span>} color="green" loading={summaryLoading} />
        </div>

        {/* Stat cards row 2 */}
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard title="Signals / sec" value={summary?.signals_per_second ?? 0} subtitle="Ingestion rate" icon={<span className="text-lg">⚡</span>} color="cyan" loading={summaryLoading} />
          <StatCard title="Queue Backlog" value={summary?.queue_backlog ?? 0} subtitle="Pending signals" icon={<span className="text-lg">◎</span>} color="purple" loading={summaryLoading} />
